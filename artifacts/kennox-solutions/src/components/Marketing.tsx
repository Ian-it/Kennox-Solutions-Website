...
export function ArtPanel({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={`relative overflow-hidden rounded-[1.75rem] bg-[hsl(var(--primary))] ${compact ? 'h-[220px]' : 'h-[320px] sm:h-auto sm:min-h-[420px] lg:min-h-[480px]'} p-5 sm:p-6 text-[hsl(var(--primary-foreground))]`}>
    ...
    <div className="absolute inset-x-5 bottom-5 z-10 sm:inset-x-6 sm:bottom-6">
      ...
      <p className="serif mt-1.5 max-w-[17rem] text-xl italic leading-tight sm:text-2xl">Turning ideas into meaningful solutions.</p>
    </div>
  </div>;
}
